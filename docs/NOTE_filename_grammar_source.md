...
- `Date` - Follows immediately after site name, in the format of either `YYYY-MM-DD` or `YY-MM-DD`
- `Actor(s)`
...
+ **StudioID:** Typically used when sites host many small, independent studios.
...
## Manual Add
*If the studio is not yet supported, you can add the scene manually*
+ **Add** Keyword Add means you want to add the scene manually
+ **Date** You can add the date but is not required
+ **Actor** Adds actor. You can add multiple actors by typing 'And' between each one
+ **Title** After actor(s), keyword 'In' instructs that scene title follows
+ **Studio** After title or actor(s), keyword 'At' instructs that studio follows
+ **Genres** After all above, keyword 'With' instructs what genres should be added
...
+ **Exact Match** examples:
  ...
  - A direct url match, using only a suffix:
    - `SiteName` - `Direct URL`
      - `warnerbros` - `eager-hands` (taken from the example URL https://warnerbros.com/video/eager-hands)
    - `SiteName` - `YY-MM-DD` - `Direct URL`
      - `warnerbros` - `2019.01.01` - `eager-hands` (taken from the example URL [https://www.warnerbros.com/movies/1809/eager-hands)
      - `warnerbros` - `2019.10.10` - `Stranger-Than-Fiction 77675` (taken from the example URL https://www.warnerbros.com/en/movie/Stranger-Than-Fiction/77675)
      - `warnerbros` - `2019.10.10` - `Stranger Than Fiction Scene 1 167063` (taken from the example URL [https://www.warnerbros.com/en/video/Stranger-Than-Fiction-Scene-1/167063)
...
Note:
- For parsing title, unless the file name starts with 'Add', then files names can expected to have a pattern where `<actors> with <genre> % <matching patterns from above>`. So for example:
  - `"Actress A, Actress B with scary, comedy % warnerbros - 5132234"` translates to
    - Actors = Actress A, Actress B
    - Genre = Scary, Comedy
    - Pattern Matching from above, studio = warnerbros and scene id = 5132234
